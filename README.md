<!-- =========================================================
     NOTYOGI — GitHub Profile README
     ========================================================= -->

<div align="center">

# Hey, I'm Yogendra 👋

### Software Developer • Full-Stack • AI • APIs • Developer Tools

<p>
  <a href="https://notyogi.space">
    <img src="https://img.shields.io/badge/Portfolio-notyogi.space-000000?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio"/>
  </a>
  <a href="https://www.linkedin.com/in/yogendra30/">
    <img src="https://img.shields.io/badge/LinkedIn-Yogendra%20Chaurasiya-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
  </a>
  <a href="https://x.com/notyogi_">
    <img src="https://img.shields.io/badge/X-@notyogi__-000000?style=for-the-badge&logo=x&logoColor=white" alt="X"/>
  </a>
</p>

</div>

---

## 👨‍💻 About Me

I'm a software developer interested in building **useful products, scalable web applications, AI-powered systems, and developer tools**.

I enjoy working across the stack — from designing interfaces and APIs to building databases, authentication systems, AI pipelines, and deploying production applications.

- 🔭 Building **full-stack applications and AI-powered products**
- 🤖 Exploring **Generative AI, RAG, LLM applications & AI agents**
- 🧠 Working with **LangChain, Hugging Face, embeddings & vector databases**
- 🌐 Building with **React, Next.js, TypeScript & Python**
- ⚙️ Interested in **backend engineering, APIs & system design**
- 🛠️ Creating **developer tools and practical SaaS products**
- 📚 Continuously learning, experimenting and shipping
- 💡 I like turning ideas into working products

> **Build. Break. Learn. Ship. Repeat.**

---

## 🧰 Tech Stack

### Languages

<p>
  <img src="https://skillicons.dev/icons?i=python,cpp,js,ts,html,css" />
</p>

### Frontend

<p>
  <img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,vite" />
</p>

### Backend & APIs

<p>
  <img src="https://skillicons.dev/icons?i=nodejs,express,django,flask,fastapi" />
</p>

### Databases & Backend Services

<p>
  <img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb,firebase,supabase" />
</p>

### AI / Machine Learning

<p>
  <img src="https://skillicons.dev/icons?i=tensorflow,pytorch" />
</p>

<p>
  <img src="https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=chainlink&logoColor=white" />
  <img src="https://img.shields.io/badge/HuggingFace-FFD21E?style=for-the-badge&logo=huggingface&logoColor=black" />
  <img src="https://img.shields.io/badge/RAG-6C47FF?style=for-the-badge" />
  <img src="https://img.shields.io/badge/LLMs-111111?style=for-the-badge" />
</p>

### Tools & Platforms

<p>
  <img src="https://skillicons.dev/icons?i=git,github,docker,vercel,postman,vscode" />
</p>

---

## 📊 GitHub Statistics

<div align="center">

<a href="https://github.com/notyogi">
  <img
    src="https://notyogi.vercel.app/api?username=notyogi&show_icons=true&hide_border=true&include_all_commits=true"
    height="180"
    alt="GitHub Statistics"
  />
</a>

<a href="https://github.com/notyogi">
  <img
    src="https://notyogi.vercel.app/api/top-langs/?username=notyogi&layout=compact&hide_border=true&langs_count=8"
    height="180"
    alt="Top Languages"
  />
</a>

</div>

---

## 🔥 Contribution Streak

<div align="center">

<img
  src="https://streak-stats.demolab.com?user=notyogi&hide_border=true&theme=transparent"
  alt="GitHub Contribution Streak"
/>

</div>

---
